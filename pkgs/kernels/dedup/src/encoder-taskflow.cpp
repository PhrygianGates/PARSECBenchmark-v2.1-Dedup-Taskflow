...
class ProcessBlocks {
public:
	tf::Executor *executor_ptr;
	ProcessBlocks(tf::Executor *e) : executor_ptr(e) {};
	void* operator()( void* vitem ) {
        ...
        tf::Taskflow taskflow("sub_pipeline");

        
        // create a pipeline graph
        tf::DataPipeline pl(1024,
            tf::make_data_pipe<void, void*>(tf::PipeType::SERIAL, [&](tf::Pipeflow& pf) {
                void* ret = filter1(nullptr);
                if (ret == NULL) pf.stop();
                return ret;
            }),

            tf::make_data_pipe<void*, void*>(tf::PipeType::PARALLEL, [&](void* item) {
                return filter2(item);
            }),
            ...
            tf::make_data_pipe<void*, void>(tf::PipeType::SERIAL, [&](void* item) {
                filter4(item);
            })
        );

        taskflow.composed_of(pl).name("sub_pipeline");
        executor_ptr->run_and_wait(taskflow);
...
void Encode( config *conf ) {
	tf::Executor executor(conf->nthreads);
	tf::Taskflow taskflow("dedup_pipeline");
	...
	ProcessBlocks *midfilter = new ProcessBlocks(&executor);
	...
	tf::DataPipeline pl(1024,
		tf::make_data_pipe<void, void*>(tf::PipeType::SERIAL, [&](tf::Pipeflow& pf) {
			void* ret = (*infilter)(nullptr);
			if (ret == NULL) pf.stop();
			return ret;
		}),

		tf::make_data_pipe<void*, void*>(tf::PipeType::PARALLEL, [&](void* item) {
			return (*midfilter)(item);
		}),
	...
	taskflow.composed_of(pl).name("dedup_pipeline");
